...
function AKP48(config, configOpts, reloadFunction, persist) {
  ...
  if(this.config.firebase && this.config.firebase.enabled) {
    var e = false;
    if(!this.config.firebase.projectId) {
      global.logger.error(`AKP48: Firebase config missing required parameter 'projectId'!`);
      e = true;
    }
    if(!this.config.firebase.clientEmail) {
      global.logger.error(`AKP48: Firebase config missing required parameter 'clientEmail'!`);
      e = true;
    }
    if(!this.config.firebase.privateKey) {
      global.logger.error(`AKP48: Firebase config missing required parameter 'privateKey'!`);
      e = true;
    }

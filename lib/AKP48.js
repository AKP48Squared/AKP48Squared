...
AKP48.prototype.start = function (persistentObjects) {
  if(!persistentObjects) {
    ...
    GLOBAL.logger.debug(`AKP48: Reloading bot.`);
  }
...
AKP48.prototype.reload = function () {
  ...
  for (var svr in this.svrInstances) {
    if (this.svrInstances.hasOwnProperty(svr)) {
      ...
      GLOBAL.logger.silly(`AKP48: Disconnecting and deleting ${svr}.`);
  ...
  //jshint ignore:start
  for (var plugin in this.plugins.ServerConnector) {
    if (this.plugins.ServerConnector.hasOwnProperty(plugin)) {
      GLOBAL.logger.silly(`AKP48: Deleting ${plugin}.`);
      delete self.plugins.ServerConnector[plugin];
    }
  }
  ...
  for (var plugin in this.plugins.MessageHandler) {
    if (this.plugins.MessageHandler.hasOwnProperty(plugin)) {
      ...
      this.plugins.MessageHandler[plugin].loaded.unload().then(function(){
        GLOBAL.logger.silly(`AKP48: Deleting ${plugin}.`);
        delete self.plugins.MessageHandler[plugin];
      });
    }
  }
  ...
  for (var plugin in this.plugins.BackgroundTask) {
    if (this.plugins.BackgroundTask.hasOwnProperty(plugin)) {
      GLOBAL.logger.silly(`AKP48: Unloading ${plugin}.`);
      this.plugins.BackgroundTask[plugin].loaded.unload().then(function(){
        GLOBAL.logger.silly(`AKP48: Deleting ${plugin}.`);
        delete self.plugins.BackgroundTask[plugin];
      });
    }
  }
  //jshint ignore:end
  ...
  for (var prop in require.cache) {
    if (require.cache.hasOwnProperty(prop)) {
      GLOBAL.logger.silly(`AKP48: Deleting ${prop} from require cache.`);

...
function AKP48(config, configOpts, reloadFunction, persistentObjects, startTime) {
  ...
  this.loadPlugins().then(function() {
    //start the bot.
    self.start(persistentObjects).then(function(){
...
AKP48.prototype.start = function (persistentObjects) {
  ...
  return new Promise(function(resolve) {
    ...
    for (var i = 0; i < self.config.servers.length; i++) {
      ...
      if(!self.plugins.ServerConnector[svr.plugin]) {
        ...
        var Plugin = self.plugins.ServerConnector[svr.plugin].module;
        var pluginInstance;
        try {
          if(persistentObjects) {
            pluginInstance = new Plugin(svr.config, svr._id, self, persistentObjects[svr._id]);
          } else {
            pluginInstance = new Plugin(svr.config, svr._id, self);
          }
...
AKP48.prototype.loadPlugins = function () {
  ...
  proms.push(new Promise(function(resolve, reject) {
    global.logger.debug(`AKP48: Loading plugins from plugins folder. (${path.resolve(self._configDir, 'plugins/*/plugin.json')})`);

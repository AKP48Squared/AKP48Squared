...
AKP48.prototype.sendMessage = function (pluginInstance, to, message) {
  console.log(pluginInstance, to, message);
  this.emit('msg_'+pluginInstance, to, message);
};
...
AKP48.prototype.start = function () {
  ...
  for (var i = 0; i < this.config.servers.length; i++) {
    ...
    var pluginInstance = new Plugin(svr.config);
    ...
    glob('plugins/**/plugin.json', function(err, files) {
      ...
      for (var i = 0; i < files.length; i++) {
        ...
        if(info.type !== 'ServerConnector') {
          plugin.loaded = new plugin.module();
        }

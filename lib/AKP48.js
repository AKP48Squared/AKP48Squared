...
function AKP48(config, restart) {
  ...
  this.loadPlugins().then(function() {
    //if we're not restarting, just go
    if(!restart) {
      //start the bot.
      self.start();
    } else {
      //if we are restarting, gather our old connections from the parent process first.
      process.send('getObjects');

      process.on('message', function(m, obj) {
        if(m === 'objects') {
          self.start(obj);
          process.send('endParent');
        }
      });
    }
  });
...
AKP48.prototype.reload = function () {
  ...
  //time to restart completely
  var child = require('child_process').fork('app.js', {
    env: {
      AKP48Restart: true
    }
  });
  ...
  child.on('message', function(m) {
    if(m === 'getObjects') {
      child.send('objects', persistentObjects);
    } else if (m === 'endParent') {
      child.unref();
      process.exit(0);
    }

...
const stdInMonitor = require('./StdInMonitor');
const firebase = require('firebase-admin');
jsonfile.spaces = 4;

function AKP48(config, configOpts, reloadFunction, persist) {
  ...
  this._startTime = persist.startTime || Date.now();
  this._lastReloadTime = Date.now();

  this._numReloads = persist.numReloads || 0;

  this._pluginList = {};

  this._totalMsgCount = persist.msgCount.tMsg || 0; //includes totalCmdCount.
  this._totalMsgChars = persist.msgCount.tChar || 0;
  this._totalCmdCount = persist.msgCount.tCmd || 0;
  this._totalSentMsgCount = persist.msgCount.tSent || 0;
  ...
  this.db = {
    get: function (refLoc) {
      return new Promise((resolve, reject) => {
        if(self._firebase) {
          var db = self._firebase.database();
          var ref = db.ref(refLoc);
          ref.once("value", (d) => {
            resolve(d);
          });
        } else {
          reject('No Firebase integration available.');
        }
      });
    };

    getRawRef = function (refLoc) {
      if(self._firebase) {
        var db = self._firebase.database();
        return db.ref(refLoc);
      }
      return false;
    };

    set: function (refLoc, data) {
      return new Promise((resolve, reject) => {
        if(self._firebase) {
          var db = self._firebase.database();
          var ref = db.ref(refLoc);
  ...
  this.TextDecorators = {
    default: require('./TextDecorator'),
    markdown: require('./MarkdownTextDecorator')
  };
  ...
  if(this.config.defaultConfig) {
    global.logger.fatal('AKP48: Edit your config file!');
    process.exit(0);
  }

  if(this.config.firebase && (this.config.firebase.enabled || this.config.firebase.enabled === null)) {
    var e = false;
    if(!this.config.firebase.credential) {
      global.logger.error(`AKP48: Firebase config missing required parameter 'credential'!`);
      e = true;
    }
    if(!this.config.firebase.databaseURL) {
      global.logger.error(`AKP48: Firebase config missing required parameter 'databaseURL'!`);
      e = true;
    }
    if(e) {
      global.logger.error(`AKP48: Firebase integration will be disabled. More errors may occur, if you have plugins which depend on this integration!`);
    } else {
      global.logger.info(`AKP48: Initializing Firebase integration.`);
      try {
        this._firebase.initializeApp({
          credential: this._firebase.credential.cert(this.config.firebase.credential),
          databaseURL: this.config.firebase.databaseURL
        });
      } catch (e) {
        global.logger.error(`AKP48: Firebase initialization failed. Firebase will be unavailable.`);
...
AKP48.prototype.unloadPlugins = function () {
  ...
  for (var plugin in this.plugins.Generic) {
    if (this.plugins.Generic.hasOwnProperty(plugin)) {
      plugins.push(this.plugins.Generic[plugin].loaded);
    }
  }
  ...
  }).timeout(Math.min(plugin.timeout || 500, 2000), `${plugin.name} took too long to unload`));
};
//jshint ignore:end

AKP48.prototype.shutdown = function (msg, console) {
  if(console) {
    this.sendAlert(`Shutdown triggered from console...`);
  } else {
    this.sendAlert(`Shutting down...`);
  }
...
AKP48.prototype.reload = function () {
  ...
  this.unloadPlugins().catch(Promise.TimeoutError, function(error) {
    ...
    persistentObjects.AKP48 = {
      startTime: self._startTime,
      msgCount: {
        tMsg: self._totalMsgCount,
        tChar: self._totalMsgChars,
        tCmd: self._totalCmdCount,
        tSent: self._totalSentMsgCount
      },
      numReloads: self._numReloads + 1
    };

    //time to reload
    global.logger.debug('AKP48: Reloading.');

    self.reloadFunction(persistentObjects);
  });
};

...
AKP48.prototype.getArgs = function (text) {
  if (text.indexOf('--') != -1) { // Strip out flags
    text = text.substring(0, text.indexOf('--'));
  }
  return text.match(/[^\s"]+|"(?:\\"|[^"])*"/g).map((s) => {
    if (s.match(/^"(?:\\"|[^"])*"$/)) s = s.substring(1, s.length - 1); // strip the quotes
    return s.trim().replace('\\"', '"'); // Trim just in case weird things happen...
  });
};

AKP48.prototype.getFlags = function (text) {
  var flags = {}, index = text.indexOf('--');
  if (index === -1) return flags; // We have no flags
  text = text.substring(index); // Strip out non-flags
  return flags;
};

AKP48.prototype.onMessage = function (line, data) {
  if(!line || !data) {return;} //if we don't have a context or we don't have text, don't continue.

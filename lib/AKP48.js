...
AKP48.prototype.unloadPlugins = function () {
  ...
  return Promise.map(plugins, plugin => new Promise(function(resolve) {
    GLOBAL.logger.silly(`AKP48: Unloading ${plugin._pluginName}.`);
    resolve(plugin.unload());
  }).timeout(2000, `${plugin._pluginName} took too long to unload`));//timeout makes us error out after 2000ms.
};
//jshint ignore:end
...
AKP48.prototype.reload = function () {
  ...
  this.unloadPlugins().catch(Promise.TimeoutError, function() {
    GLOBAL.logger.error('AKP48: Could not unload all plugins within 2000ms.');
  }).finally(function() {

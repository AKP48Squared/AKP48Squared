  ...
  proms.push(new Promise(function(resolve) {
    ...
    for (var i = 0; i < plugins.length; i++) {
      ...
      try {
        ...
        name = plugin.name || name; // allow plugins to override name.
        var plg = {
          conf: null,
          module: plugin,
          name: name
        ...
        self.plugins[plugin.type][name] = plg;
        global.logger.verbose(`AKP48: ${name} loaded.`);
      } catch(e) {
        global.logger.error(`AKP48: Error encountered loading ${name}! Plugin not loaded. Error: ${e.message}`);
        console.error(e.stack);
      }
    }

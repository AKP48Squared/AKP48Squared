...
AKP48.prototype.sendMessage = function (message, context) {
  ...
  var to = context.to();
  context = context.cloneWith({text: message});

  if (context.isAlert())  {
    global.logger.stupid(`AKP48: Sending alert to all instances.`);
    this.sendAlert(message);
  } else {
    global.logger.stupid(`AKP48: Sending message to ${context.instanceId()}:${to}.`);
    this.emit('msg_'+context.instanceId(), context);
  }
};
...
AKP48.prototype.loadPlugins = function () {
  ...
  (function getPluginsFromNpm() {
    ...
    for (var name in package) { // jshint ignore:line
      if (true || !package.hasOwnProperty(name) || !name.toLowerCase().startsWith('akp48-plugin-')) {
        continue;
      }
      var module;
      try {
